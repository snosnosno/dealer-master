...
- **좌석 인덱스는 끝까지 0-based, 화면에 찍는 번호는 항상 `+1`.** 변환은 **UI 계층에서는**
  `displaySeat()` 한 곳에서만 한다. 엄밀히 말해 전역 유일 변환점은 아니다 — 동결된 엔진의
  `decisions.ts:106` 이 문항 `sub` 텍스트에서 자체적으로 `hand.buttonSeat + 1` 을 한다.
  형식이 `이름(N번)` 으로 `log.ts` 의 `who()` 와 정확히 일치해 화면상 불일치는 없다.
  엔진 수정 금지 제약 아래에서는 이대로 두고, UI 쪽 변환점만 하나로 유지한다
...
- [ ] 폴드한 좌석이 흐려진다
- [ ] **좌석 박스가 서로 겹치지 않는다 — 특히 SB·BB 가 둘 다 벳 칩 줄을 띄운 프리플랍
      순간을 6-max 에서 확인한다.** (Task 4 리뷰 지적: 인접 좌석 세로 간격이 dy≈93.5px 인데
      벳 줄이 보이는 좌석의 높이 추정치가 ~94px 라 경계선이다. 폰트 메트릭·라인하이트·
      패딩 반올림이 걸려 있어 렌더 없이는 판정 불가)
- [ ] **좌석 박스가 타원 좌표점에 세로로 중심 정렬돼 보인다.** (Task 4 리뷰 지적:
      가로 `-48` 은 `w-24`=96px 의 정확한 절반이지만 세로 `-30` 은 고정값이라, 벳 줄
      유무에 따라 좌석이 의도한 점보다 아래로 밀려 보일 수 있다. 미용 문제)
...
- [ ] **로그가 페이지 전체를 스크롤시키지 않는다.** 뷰포트 높이를 ~667px 로 줄여 로그가
      화면 아래로 반쯤 걸치게 한 뒤 자동 재생한다. 로그 **내부** 스크롤은 정상이지만
      **페이지**가 줄마다 따라 움직이면 결함이다. (Task 5 리뷰 지적: `scrollIntoView` 는
      명세상 스크롤 가능한 **모든 조상**을 움직인다. `block:'nearest'` 는 "이미 보이면
      가만히" 라서 로그가 뷰포트 안에 완전히 들어와 있을 때만 무해하다.)
      고치게 되면 좁은 수정은 sentinel 대신 컨테이너를 직접 스크롤하는 것 —
      ref 로 `scrollTop = scrollHeight`. 조상을 건드릴 수 없다. `inline:'nearest'` 는
      축이 달라 도움이 안 된다
- [ ] **스크린리더에서 가장 빠른 속도로 재생했을 때 로그 줄이 통째로 건너뛰어지는지**
      확인하고, 안정적으로 읽히는 최대 속도를 기록한다. (Task 5 리뷰 지적: `aria-live`
      는 갱신이 음성보다 빠르면 큐를 덮어쓴다. 속성으로는 못 고친다 — `assertive` 는
      더 많이 끊어먹는다. 실제 완화책은 재생 컨트롤 쪽 제품 결정이므로 3단계로 넘긴다)
