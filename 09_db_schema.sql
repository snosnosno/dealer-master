-- ⚠️ 보류 — 이 스키마는 현재 방향을 담지 못한다 (PRD.md §9).
-- 기록 층은 로그인과 함께 뒤로 미뤄졌고(PRD §4), 되살릴 때 아래 결함을 먼저 고쳐야 한다:
--   1) attempts.case_id NOT NULL — 생성 문제(러시·액션)는 cases 행이 없어 저장할 수 없다
--   2) cases.track 체크 제약에 러시 축이 빠져 있다
--   3) attempts.mode 의 'timeline' 은 ADR-002 로 폐기된 C모드 잔재다
--   4) streaks 가 주 단위 — 러시는 "매일 3분" 리듬이다
-- 해법은 PRD §9 에 결정돼 있다. 실행 전 반드시 읽을 것.
